 ...
 * \brief		FatFs module is a generic FAT file system module
 * \{
 *
 * \brief		FatFs - FAT file system module
 *
 * \file		ff.c
 * \version		R0.10
 * \date		2013-10-02
...
/**
 *****************************************************************************
 * \defgroup 	FatFs_Core_Private_Definitions Private definitions
 * \brief		Module Private Definitions
 * \{
 *****************************************************************************
 */
...
#endif
/** \brief	Enter in a thread-safe state.									*/
#define	ENTER_FF(fs)		{ if (!lock_fs(fs)) return FR_TIMEOUT; }
/** \brief	Exit from a thread-safe state.									*/
...
#define IsUpper(c)	(((c)>='A')&&((c)<='Z'))	/**< Is char a upper letter	*/
#define IsLower(c)	(((c)>='a')&&((c)<='z'))	/**< Is char a lower letter	*/
#define IsDigit(c)	(((c)>='0')&&((c)<='9'))	/**< Is char a digit		*/

#if _DF1S		/* Code page is DBCS */

#ifdef _DF2S	/* Two 1st byte areas */
/**
 * \brief	1st byte area(s)
 */
...
#define IsDBCS1(c)	0		/**< 1st byte area(s)							*/
#define IsDBCS2(c)	0		/**< 2nd byte area(s)							*/
...
#define BPB_HiddSec			28		/**< Number of special hidden
										 sectors (4)						*/
...
#define	DDE					0xE5	/**< Deleted directory entry mark in
										 DIR_Name[0]						*/
#define	NDDE				0x05	/**< Replacement of the character
										 collides with DDE					*/

/**
 * \defgroup 	FatFs_Core_Private_Workarea Private work area
 * \brief		Module private work area
 * \note		Note that uninitialized variables with static duration are
...
#if _VOLUMES
static FATFS *FatFs[_VOLUMES];	/**< Pointer to the file system objects
									 (logical drives)						*/
#else
...
#elif _USE_LFN == 2 		/**< LFN feature with dynamic working buffer on
								 the stack									*/
#define	DEF_NAMEBUF			BYTE sfn[12]; WCHAR lbuf[_MAX_LFN+1]
#define INIT_BUF(dobj)		{ (dobj).fn = sfn; (dobj).lfn = lbuf; }
#define	FREE_BUF()

#elif _USE_LFN == 3 		/**< LFN feature with dynamic working buffer on
								 the heap									*/
...
#ifdef _EXCVT
static const BYTE ExCvt[] = _EXCVT; /**< Upper conversion table for extended
										 characters							*/
#endif
...
/**
 *****************************************************************************
 * \defgroup 	FatFs_Core_Private_Functions Private functions
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_String String functions
...
/**
 * \brief	Fill memory
 * \param	dst		Memory buffer
 * \param	src		Value to fill in
 * \param	cnt		Count of bytes
 */
 ...
 * \param	src		Source string
 * \param	cnt		Count of bytes
 * \return	0 if memories are the same
 */
...
/**
 * \brief	Check if chr is contained in the string
 * \param	str		Search string
 * \param	chr		Pattern to search
 * \return	0 if chr isn't contained in the string
 */
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_Reentrant Reentrant functions
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_FS_Lock FS Lock functions
...
#if _FS_LOCK
/**
 * \brief	Increment object open counter and returns its index
 *			(0:Internal error)
 * \param	dp		Directory object pointing the file to register or
 *					increment
 * \param	acc		Desired access
 *					\arg 0:	Read
 *					\arg 1:	Write
 * \return	0 if internal error, else its index
 */
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_Window Window functions
 * \brief		Move/Flush disk access window in the file system object
 * \{
 */
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_Sync Synchronization functions
 ...
 * \brief	Synchronize file system and storage device
 * \param	fs		File system object
 * \return	\arg FR_OK:			successful\n
 *			\arg FR_DISK_ERR:	failed
 */
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_SectorCluster Sector-Number and Cluster-Nbr
 ...
 * \return	\arg !=0:	Sector number
 *			\arg 0:		Failed - invalid cluster#
 */
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_Fat_Access FAT access
 * \brief		FAT access
 * \{
 */
 ...
 * \return	\arg 0xFFFFFFFF:	Disk error
 *			\arg 1:				Internal error
 *			\arg else:			Cluster status
 */
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_Fat_Handling FAT handling
 * \brief		FAT handling
 * \{
 */
...
#if !_FS_READONLY
/**
 * \brief	Stretch or Create a cluster chain
 * \param	fs		File system object
 * \param	clst	Cluster# to stretch. 0 means create a new chain.
 * \return	\arg 0:				No free cluster
 *			\arg 1:				Internal error
 *			\arg 0xFFFFFFFF:	Disk error
 *			\arg >=2:			New cluster#
 */
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_Directory_Handling Directory handling
 * \brief		FAT handling
 * \{
 */
 ...
 * \param	dp		Pointer to directory object
 * \param	stretch	\arg 0:	Do not stretch table\n
 *					\arg 1:	Stretch table if needed
 * \return	\arg FR_OK:			Succeeded
 *			\arg FR_NO_FILE:	End of table
 *			\arg FR_DENIED:		Could not stretch
 */
...
/**
 * \brief	Load/Store start cluster number
...
/**
 * \defgroup 	FatFs_Core_Private_Functions_LFN_Handling LFN handling
 * \brief		Test/Pick/Fit an LFN segment from/to directory entry
 * \{
 */

#if _USE_LFN
/**
 * \brief	Offset of LFN characters in the directory entry
 */
 ...
 * \param	dir		Pointer to the directory entry
 * \return	\arg 1:	Succeeded
 *			\arg 0:	Buffer overflow
 */
static int pick_lfn (
		WCHAR* lfnbuf,
		BYTE* dir
...
/**
 * \addtogroup 	FatFs_Core_Private_Functions_Directory_Handling Directory handling
 * \{
 */

/**
 * \brief	Find an object in the directory
 ...
 * \return	\arg FR_OK:			Successful
 *			\arg FR_DENIED:		No free entry or too many SFN collision
 *			\arg FR_DISK_ERR:	Disk error
 */
...
/**
 * \brief	Get logical drive number from path name
...
/**
 * \brief	Load a sector and check if it is an FAT boot sector
 * \param	fs		File system object
 * \param	sect	Sector# (lba) to check if it is an FAT boot record or not
 * \return	\arg 0:	FAT boor sector
 *			\arg 1:	Valid boor sector but not FAT
 *			\arg 2:	Not a boot sector
 *			\arg 3:	Disk error
 */
...
/**
 * \brief	Find logical drive and check if the volume is mounted
 ...
 * \return	\arg FR_OK(0):	The object is valid
 *			\arg !=0:		Invalid
 */
 ...
 * \param	opt		\arg 0: Do not mount (delayed mount)\n
 *					\arg 1: Mount immediately
 * \return	#FRESULT
 */
 ...
 * \param	mode	Access mode and file open mode flags, see
 *					\ref FatFs_Core_Public_FIL_flag
 * \return	#FRESULT
 */
 ...
 * \param	fatfs	Pointer to return pointer to corresponding file system
 *					object
 * \return	#FRESULT
 */
...
#define N_ROOTDIR	512		/**< Number of root directory entries for
								 FAT12 / FAT16								*/
 ...
 * \param	pb		Pointer to the buffer object
